...
statesData.features.forEach(function (stateData) {
  ...
  const promise = d3.json(`${url}&locationdesc=${state}`).then(function (data) {
    ...
    if (densities.length) {
      ...
      average =
        densities.reduce(function (sum, density) {
          return sum + density;
        }, 0) / densities.length;
    }
...
Promise.all(promises).then(function () {
  ...
  function getColor(d) {
    ...
    for (
      let value = max - step, i = colors.length - 1;
      value >= min;
      value -= step, i--
    ) {
      if (d >= value) {
  ...
  info.update = function (props) {
    this._div.innerHTML =
      "<h4>US Average Obesity Levels</h4>" +
      (props
        ? "<b>" +
          props.name +
          "</b><br />" +
  ...
  legend.onAdd = function () {
    ...
    let legendInfo =
      "<h3>Adult Population with Obesity</h3>" +
      '<div class = "labels">' +
      '<div class = "min">' +
      limits[0] +
      "</div>" +
      '<div class = "max">' +
      limits[limits.length - 1].toLocaleString() +
      "</div>";
    ("</div>");
    ...
    limits.forEach(function (limit, index) {
      ...
      labels.push('<li style="background: ' + colors[index] + '"></li>');
    });
